...
<head>
    ...
    function getHisAgentList() {
		...
		var params = {
				'class' : 'eduMapper',
				'method' : 'getHisAgentList',
				'param' : {'AGT_ID' : AGT_ID},
				'callback' : function(data){
					if(data.success){
						loadingStop();
						
						$('#hisAgentList .items').remove();
						var _html = '';
						if(data.list.length > 0) {
							$.each(data.list, function(index, item) {
								_html += '<tr class="items" data-chng_id="'+item.CHNG_ID+'">'
									...
									_html += '<td><a href="/edu/hisAgentDetail?CHNG_ID='+item.CHNG_ID+'" class="reply_btn">보기</a></td>'
									_html += '<td class="t_left">'+item.AGT_NOTE+'</td>'
								_html += '</tr>'
							});
						} else {
				...
				<div class="sub_box">
					...
					<div class="sub_con">
						...
						<div class="conBox">
							<div class="con_title">
								교육기관 / 
								<span class="con_sub_title_ty2"><span id="AGT_NAME"></span> 정보 변경 이력</span>
							</div>
							<div class="con_form">
								<table class="tbl_ty2" id="hisAgentList">
									<colgroup>
										<col width="80px"/>
										<col width="150px"/>
										<col width="150px"/>
