...
<head>
    ...
    function getHisStudentList() {
		...
		var params = {
				'class' : 'eduMapper',
				'method' : 'getHisStudentList',
				'param' : {'STU_ID' : STU_ID},
				'callback' : function(data){
					if(data.success){
						loadingStop();
						
						$('#hisStudentList .items').remove();
						var _html = '';
						if(data.list.length > 0) {
							$.each(data.list, function(index, item) {
								_html += '<tr class="items" data-chng_id="'+item.CHNG_ID+'">'
									...
									_html += '<td><a href="/edu/hisStudentDetail?CHNG_ID='+item.CHNG_ID+'" class="reply_btn">보기</a></td>'
									_html += '<td class="t_left">'+item.STU_NOTE+'</td>'
								_html += '</tr>'
							});
						} else {
				...
				<div class="sub_box">
					...
					<div class="sub_con">
						...
						<div class="conBox">
							<div class="con_title">
								화면설계 교육기관 / 
								<span class="con_sub_title_ty2">교육생 - <span id="STU_NAME"></span> 정보 변경 이력</span>
							</div>
							<div class="con_form">
								<table class="tbl_ty2" id="hisStudentList">
									<colgroup>
										<col width="80px"/>
										<col width="150px"/>
										<col width="150px"/>
