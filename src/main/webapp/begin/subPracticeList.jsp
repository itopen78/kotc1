...
<head>
	...
	$(document).ready(function() {
		...
		$('#gridList').on('click', '.setGridFn', function() {
			location.href = '/begin/subPracticeChangeList?CLASS_ID='+$(this).data('class_id')+'&CLASS_TITLE='+$(this).data('class_title');
		});
	...
	function getSubPracticeList(index) {
		...
		var params = {
				'class' : 'service.BeginService',
				'method' : 'getSubPracticeList',
				'param' : {
					'S_WRITED_DATE' : _S_WRITED_DATE,
					'E_WRITED_DATE' : _E_WRITED_DATE,
					'LOGIC_CLASS_TITLE' : _LOGIC_CLASS_TITLE,
					...
					if(data.success){
						...
						if(data.list.length > 0) {
							$.each(data.list, function(index, item) {
								_html += '<tr class="items" data-class_id="'+item.CLASS_ID+'">'+
											'<td>'+item.NO+'</td>'+
											'<td>'+item.WRITED_DATE+'</td>'+
											'<td>'+item.LOGIC_CLASS_TITLE+'</td>'+
											
											'<td class="t_left"><a href="javascript:void(0);" class="setGridFn" data-class_id="'+item.CLASS_ID+'" data-class_title="'+item.CLASS_TITLE+'">'+item.CLASS_TITLE+'</a></td>'+
											'<td>'+item.USER_NAME+'</td>'+
											'<td>'+item.CHNG_DATE+'</td>'+
											'<td>'+
	
												'<ul class="tbl_btn">'+
													'<li><a href="javascript:void(0);" class="reply_btn changeBtn" data-class_id="'+item.CLASS_ID+'">변경</a></li>'+
					...
					<div class="sub_menu">
						...
						<div class="sub_list">
							<ul>
								...
								<li class="sub_on"><a href="/begin/subPracticeList">대체실습</a></li>
							</ul>
						</div>
					</div>
					<div class="sub_con">
							...
							<div class="con_form">
								<table class="tbl_ty3">
									...
									<tr>										
										...
										<td>
											<input type="text" class="tbl_input wd130" id="LOGIC_CLASS_TITLE" name="LOGIC_CLASS_TITLE"/>
										</td>
										<td>
						...
						<div class="conBox">
							...
							<div class="con_form mgt10">
								...
								<div class="btn_box">
									...
									<a href="/begin/logicWrite" class="btn_right wd100">신규등록</a>									
								</div>
