	...
	***********************************************************************/
	...
	$(document).ready(function() {
		...
		$('#gridList').on('click', '.setGridFn', function() {
			var _CHNG_ID = $(this).closest('tr.items').data('chng_id');	
			var _AGC_ID = $(this).closest('tr.items').data('agc_id');
				
			location.href = '/begin/hisLogicStepDetail?CHNG_ID='+_CHNG_ID+'&CLASS_ID='+_CLASS_ID+'&AGC_ID='+_AGC_ID;
		});
	...
	function getLogicChangeList(index) {

		_CLASS_ID = "${CLASS_ID}";

		var params = {
				'class' : 'service.BeginService',
				'method' : 'getLogicChangeList',
				'param' : {
					'CLASS_ID' : _CLASS_ID
				},
					...
					if(data.success){
						...
						if(data.list.length > 0) {
							$.each(data.list, function(index, item) {
								...
								_html += '<tr class="items" data-chng_id="'+item.CHNG_ID+'"  data-agc_id="'+item.AGC_ID+'" >'+
								'<td>'+item.NO+'</td>'+
